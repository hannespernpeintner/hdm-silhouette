...
{
    public class PlayerHangingState2 : PlayerState
    {
        private MoveStack moves = new MoveStack();

        public PlayerHangingState2(Tom tom, Animation animationLeft, Animation animationRight) 
        ...
        public override void onSet(Tom.FacingState facingState)
        {
            ...
            moves.push(new Move.Move(tom, Vector2.Zero, 1000));
        }

        public override void handleInput(KeyboardState cbs, KeyboardState oks)
        {

            if (cbs.IsKeyDown(Keys.Space))
            {
